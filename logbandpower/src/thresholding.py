...
def generate_neuroevent():
	...
	new_msg.type = "Custom"
	new_msg.description = "Threshold of {} exceeded in channel {}".format(thr, ch)
	new_msg.value = 1 # irrelevant
...
def main():
	...
	ch = rospy.get_param('channel', 7) # channel
	thr = rospy.get_param('threshold', 1.2) # threshold
	...
	while not rospy.is_shutdown():
		...
		if new_data:
			...
			if apply_threshold(current_frame) == 1:
				...
				rospy.loginfo("Event Published: {}".format(event_msg.description))
