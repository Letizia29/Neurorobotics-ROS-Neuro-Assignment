...
def buffered_bandpower(data: NeuroFrame):
	...
	if(seq * data.eeg.info.nsamples >= data.sr):
		...
		for index in range(data.eeg.info.nchannels):
			...
			current_row = np.average(current_row)
			avgs[index] = np.log10(current_row)
...
def generate_new_message(data, old_message):
	...
	new_msg.eeg.info.nsamples = old_message.eeg.info.nsamples
	new_msg.eeg.info.nchannels = old_message.eeg.info.nchannels
	new_msg.eeg.info.stride = old_message.eeg.info.nchannels
	new_msg.sr = old_message.sr
	...
	new_msg.exg = old_message.exg
	new_msg.tri = old_message.tri
...
def main():
	...
	while not rospy.is_shutdown():
		...
		if new_data:
			...
			new_msg = generate_new_message(new_data, current_frame)
			pub.publish(new_msg)
			new_data = False
		#rate.sleep()
